...
**Offline / registry-blocked machines**: the spawn configs fetch `@rolepod/uiproof` via `npx`, which needs npm-registry access. On an air-gapped or registry-blocked host, install once with `npm i -g @rolepod/uiproof@0.22.0` and set the MCP `command` to the global `rolepod-uiproof` binary (drop the `npx` / `-y` args) so no fetch is required at spawn time.
...
```bash
npm_config_audit=false npx -y @rolepod/uiproof@0.22.0 --help
```
...
```jsonc
{
  ...
  "mcp": {
    "servers": {
      "rolepod-uiproof": {
        "type": "local",
        "command": ["npx", "-y", "@rolepod/uiproof@0.22.0"],
